...
*/
var CmsArticleRefFromCommon = {
    //表单默认值
    defaultModel: {
        //id//不需要此项
        articleId: 0,		//文章Id
        refArticleId: 0,		//关联文章Id
    },
    data: {
		editLoading:false,
        //表单验证规则
            ...
            name: [
                {required: true, message: '名称不能为空', trigger: 'blur'},
                {
                    validator: function(rule, value, callback){
                        return commonUtils.formRowIsExist('/api/admin/cms/cmsArticleRef/isExist.do',"name",rule, value, callback);
                    },
                    trigger: 'blur'
                }
            ],
    ...
    saveAction: function (_self) {
        ...
        $.ajax({
            ...
            if (res.success) {
                ...
                window.location.href = "#/cmsArticleRef";//跳转到列表页面
    ...
    closeDialogForm:function() {
		//如果是弹窗模式，关闭弹窗
        if(typeof(cmsArticleRefView) !== 'undefined' && cmsArticleRefView.dialogFormVisible) {			
			cmsArticleRefView.dialogFormClose();
		}
    }
};