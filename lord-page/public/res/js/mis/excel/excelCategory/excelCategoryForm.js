...
var ExcelCategoryFromCommon = {
               ...
               name: [
                   {required: true, message: '名称不能为空', trigger: 'blur'},
                   {
                       validator: function(rule, value, callback){
                           return commonUtils.formRowIsExist('/api/admin/excel/excelCategory/isExist.do',"name",rule, value, callback);
                       },
                       trigger: 'blur'
                   }
               ],
    ...
    loadSelect: function (_self) {
        ...
        $.ajax({
            ...
            if (res.success) {
                _self.excelCategoryStatus = res.data.mis_MisUserStatus;
            } else {
    ...
    saveAction: function (_self) {
        ...
        $.ajax({
            method: "post",
            url: '/api/admin/excel/excelCategory/saveOrUpdate.do',
            data: _self.editForm,
            dataType: "json"
        }).done(function (res, status, xhr) {
            if (res.success) {
                _self.$message.success(res.msg);//保存成功
                window.location.href = "#/excelCategory";//跳转到列表页面
    ...
    closeDialogForm:function() {
		//如果是弹窗模式，关闭弹窗
        if(typeof(excelCategoryView) !== 'undefined' && excelCategoryView.dialogFormVisible) {			
			excelCategoryView.dialogFormClose();
		}
    }
};